...
# 设置日志记录器
logger = setup_logger(log_dir="logs", log_filename="training.log")
...
# 初始化模型
net = improve_Net(KEEP_PROB_RATE).to(device)
logger.info("Improved model initialized.")
...
for epoch in range(1, EPOCHS + 1):
    ...
    if testAccuracy > best_accuracy:
        ...
        torch.save(net.state_dict(), "weights/best_improved_mnist.pth")
...
# **训练结束后，保存最后一轮权重**
torch.save(net.state_dict(), "weights/last_improved_mnist.pth")

# 训练完成后绘图
plot_metrics(history, save_path="logs/img/improve_training_curve.png")
